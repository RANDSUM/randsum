...
/**
 * Static documentation for every modifier notation surface.
 *
 * This is **pure data** — no schema logic, no dice-pool behavior, no roll engine.
 * It is the single source the `./docs` subpath derives from. Keeping it separate
 * from the `notation/definitions` schemas is deliberate: the tokenize/validate/
 * parse path imports those schemas, and co-locating this heavy doc data on them
 * would leak it into `dist/tokenize.js`.
 *
 * Docs are grouped per modifier (mirroring RANDSUM_MODIFIERS). Most modifiers own
 * a single surface; a few own several (`drop` → L/H/D{..}, `keep` → K/KL/KM,
 * `reroll` → R{..}/ro{..}, `explodeSequence` → !s{..}/!i/!r), and `count` owns
 * the whole count family (`#{..}`, `S{..}`, `F{..}`, `ms{..}`).
 */

export const capDocs: readonly NotationDoc[] = [
  {
    key: 'C{..}',
    category: 'Clamp',
    color: '#67e8f9',
    ...
    examples: [
      ...
      },
      {
        ...
        notation: '4d20C{<3,>18}',
        options: { sides: 20, quantity: 4, modifiers: { cap: { lessThan: 3, greaterThan: 18 } } }
      }
    ]
  }
]

export const replaceDocs: readonly NotationDoc[] = [
  {
    key: 'V{..}',
    category: 'Map',
    color: '#2dd4bf',
...
export const rerollDocs: readonly NotationDoc[] = [
  {
    key: 'R{..}',
    category: 'Substitute',
    color: '#f472b6',
...
export const explodeDocs: readonly NotationDoc[] = [
  {
    key: '!',
    category: 'Generate',
    color: '#fbbf24',
...
export const compoundDocs: readonly NotationDoc[] = [
  {
    key: '!!',
    category: 'Accumulate',
    color: '#f59e0b',
...
export const penetrateDocs: readonly NotationDoc[] = [
  {
    key: '!p',
    category: 'Accumulate',
    color: '#d97706',
...
export const explodeSequenceDocs: readonly NotationDoc[] = [
  {
    key: '!s{..}',
    category: 'Generate',
    color: '#fcd34d',
...
export const wildDieDocs: readonly NotationDoc[] = [
  {
    key: 'W',
    category: 'Dispatch',
    color: '#facc15',
...
export const uniqueDocs: readonly NotationDoc[] = [
  {
    key: 'U',
    category: 'Substitute',
    color: '#5eead4',
...
export const dropDocs: readonly NotationDoc[] = [
  {
    key: 'L',
    category: 'Filter',
    color: '#fb7185',
...
export const keepDocs: readonly NotationDoc[] = [
  {
    key: 'K',
    category: 'Filter',
    color: '#fb923c',
  ...
  },
  {
    ...
    displayBase: 'KM',
    forms: [{ notation: 'KM(n)', note: 'Keep middle n (default: pool - 2)' }],
    examples: [
      { description: 'Keep 3 middle dice', notation: '5d6KM3' },
      {
...
export const countDocs: readonly NotationDoc[] = [
  {
    key: '#{..}',
...
export const multiplyDocs: readonly NotationDoc[] = [
  {
    key: '*',
    category: 'Scale',
    color: '#a3e635',
...
export const plusDocs: readonly NotationDoc[] = [
  {
    key: '+',
    category: 'Scale',
    color: '#4ade80',
...
export const minusDocs: readonly NotationDoc[] = [
  {
    key: '-',
    category: 'Scale',
    color: '#f87171',
...
export const integerDivideDocs: readonly NotationDoc[] = [
  {
    key: '//',
    category: 'Scale',
    color: '#34d399',
...
export const moduloDocs: readonly NotationDoc[] = [
  {
    key: '%',
    category: 'Scale',
    color: '#10b981',
...
export const sortDocs: readonly NotationDoc[] = [
  {
    key: 'sort',
    category: 'Order',
    color: '#94a3b8',
...
export const multiplyTotalDocs: readonly NotationDoc[] = [
  {
    key: '**',
    category: 'Scale',
    color: '#84cc16',
...
/**
 * Modifier name → its doc surface(s). Re-establishes the modifier↔docs linkage
 * explicitly now that docs no longer live on the schema/modifier objects. Keyed
 * by the same `name` used in RANDSUM_MODIFIERS. A doc group's first entry is the
 * modifier's primary surface (its category classifies the modifier).
 */
export const MODIFIER_DOCS_BY_NAME: Readonly<Record<string, readonly NotationDoc[]>> = {
  cap: capDocs,
  replace: replaceDocs,
  reroll: rerollDocs,
  explode: explodeDocs,
  compound: compoundDocs,
  penetrate: penetrateDocs,
  explodeSequence: explodeSequenceDocs,
  wildDie: wildDieDocs,
  unique: uniqueDocs,
  drop: dropDocs,
  keep: keepDocs,
  ...
  plus: plusDocs,
  minus: minusDocs,
  integerDivide: integerDivideDocs,
  modulo: moduloDocs,
  sort: sortDocs,
  multiplyTotal: multiplyTotalDocs
}

/**
 * All modifier docs in RANDSUM_MODIFIERS priority order.
 */
export const MODIFIER_NOTATION_DOCS: readonly NotationDoc[] = [
  ...capDocs,
  ...replaceDocs,
  ...rerollDocs,
  ...explodeDocs,
