...
import { ModifierError, ValidationError } from '../../src/errors'
import type { RandomFn } from '../../src/index'

describe('edge cases', () => {
  describe('validation errors', () => {
    test('zero quantity throws', () => {
      expect(() => roll({ sides: 6, quantity: 0 })).toThrow(ValidationError)
    })

    test('negative quantity throws', () => {
      expect(() => roll({ sides: 6, quantity: -1 })).toThrow(ValidationError)
    })

    test('zero sides throws', () => {
      expect(() => roll({ sides: 0, quantity: 1 })).toThrow(ValidationError)
    })

    test('negative sides throws', () => {
      expect(() => roll({ sides: -5, quantity: 1 })).toThrow(ValidationError)
    })

    test('non-integer quantity throws', () => {
      expect(() => roll({ sides: 6, quantity: 0.5 })).toThrow(ValidationError)
    })

    test('non-integer sides throws', () => {
      expect(() => roll({ sides: 6.5, quantity: 1 })).toThrow(ValidationError)
    })
  ...
  describe('modifier edge cases', () => {
    test('drop all dice throws modifier error', () => {
      expect(() => roll({ sides: 6, quantity: 2, modifiers: { drop: { lowest: 2 } } })).toThrow(
        ModifierError
      )
    })

    test('drop more than available throws modifier error', () => {
      expect(() =>
    ...
    test('unique modifier throws when it cannot find unique values within attempt limit', () => {
    ...
    test('keep more than available throws modifier error', () => {
      expect(() => roll({ sides: 6, quantity: 3, modifiers: { keep: { highest: 5 } } })).toThrow(
        ModifierError
      )
    })

    test('keep zero throws modifier error', () => {
      expect(() => roll({ sides: 6, quantity: 3, modifiers: { keep: { highest: 0 } } })).toThrow(
        ModifierError
      )
    })

    test('reroll value outside range throws modifier error', () => {
