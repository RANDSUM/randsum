...
describe('countFailures modifier', () => {
  describe('via options object (using count)', () => {
    ...
    test('returns 0 when no dice are at or below threshold', () => {
      ...
      const results = Array.from({ length: 100 }, () =>
        roll({ sides: 6, quantity: 3, modifiers: { count: { lessThanOrEqual: 0 } } })
      )
      results.forEach(r => {
    ...
    test('counts all dice when threshold >= max face', () => {
      ...
      const results = Array.from({ length: 100 }, () =>
        roll({ sides: 6, quantity: 4, modifiers: { count: { lessThanOrEqual: 6 } } })
      )
      results.forEach(r => {
  ...
  describe('via notation string (F{} sugar)', () => {
