...
async function main(): Promise<void> {
  ...
  for (const specFile of specFiles) {
    ...
    shortcodes.push(spec.shortcode)
  ...
  // Emit AVAILABLE_GAMES derived from the set of spec files discovered above.
  // Sorted so the output is stable regardless of filesystem ordering.
  const sortedShortcodes = [...shortcodes].sort()
  const availableGamesSource =
  ...
  if (checkMode) {
    ...
    if (formattedAvailable !== existing) {
      stale.push('availableGames.generated.ts')
      console.error(`  STALE: availableGames.generated.ts`)
    } else {
      console.log(`  OK: src/availableGames.generated.ts`)
    }
  } else {
    writeFileSync(availableGamesPath, formattedAvailable, 'utf-8')
    console.log(`  codegen: AVAILABLE_GAMES → src/availableGames.generated.ts`)
  }
