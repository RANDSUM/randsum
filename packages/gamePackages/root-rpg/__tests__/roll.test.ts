...
describe('roll', () => {
  ...
  describe('modifiers', () => {
    test('correctly applies positive modifier', () => {
      ...
      const rawTotal = rolls[0]?.modifierHistory.initialRolls.reduce((sum, roll) => sum + roll, 0)
  ...
  describe('edge cases', () => {
    ...
    test('throws error for NaN modifier', () => {
      expect(() => roll(NaN)).toThrow('Root RPG bonus must be a finite number, received: NaN')
    })

    test('throws error for Infinity modifier', () => {
      expect(() => roll(Infinity)).toThrow(
