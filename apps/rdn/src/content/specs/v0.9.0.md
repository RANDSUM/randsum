...
This is a draft release. The specification is believed to be stable for implementation but may undergo revisions before the 1.0 release. Feedback is welcome via the [project repository](https://github.com/RANDSUM/randsum).
...
## 11. Implementing This Specification

### 11.1 Overview

This section provides guidance for developers implementing an RDN-compliant parser in any programming language. The reference implementation is the `@randsum/roller` TypeScript package ([source](https://github.com/RANDSUM/randsum/tree/main/packages/roller)).

### 11.2 Recommended Implementation Order

Implementations SHOULD follow this order to incrementally pass conformance levels:

1. **Lexer/tokenizer** — split notation into tokens: quantity, separator, sides, modifier suffixes
2. **Core dice expression** (NdS) — handle quantity, sides, and the `d` separator
3. **Stage 3 modifiers** — arithmetic operations (+, -, \*, //) are the simplest transformations
4. **Stage 2 modifiers** — pool manipulation (drop, keep, reroll, explode) requires tracking individual dice
5. **Stage 1 modifiers** — value remapping (cap, replace) runs before pool manipulation
6. **Special dice** — percentile (d%), Fate (dF), custom faces (d{...}), geometric (gN), draw (DDN)
7. **Aliases and sugar** — K, kl, S{}, F{}, ms{}, ro{}, \*\* as sugar over primitives

### 11.3 Conformance Test Suite

A machine-readable test suite is available for download:

**[Download conformance vectors (JSON)](/conformance/v0.9.0.json)**

The JSON file contains all 48 test vectors from Appendix G with structured fields:

- `notation` — the input notation string
- `seedRolls` — deterministic dice values to use (replace your RNG)
- `expectedPool` — the dice pool after all modifiers
- `expectedTotal` — the final scalar result
- `section` — the spec section that defines the behavior
- `conformanceLevel` — the minimum level that requires this vector

To use: configure your implementation to accept a deterministic random function, feed each vector's `seedRolls` as the RNG output, and compare your pool and total against the expected values.

### 11.4 Claiming Conformance

Implementations MAY claim conformance at a specific level (see Section 10) by passing all vectors for that level and all levels below it. Error case vectors (28-30, 46-48) are REQUIRED at all levels.

A conformance claim takes the form: "RDN v0.9.0 Level N Conformant" where N is 1-4.

### 11.5 Reference Implementation

The canonical reference implementation is `@randsum/roller`:

- **Source**: [github.com/RANDSUM/randsum/tree/main/packages/roller](https://github.com/RANDSUM/randsum/tree/main/packages/roller)
- **npm**: `@randsum/roller`
- **Subpath exports**: `@randsum/roller/roll` (roll function), `@randsum/roller/validate` (validation), `@randsum/roller/tokenize` (tokenizer)

The roller accepts a custom random function via `RollConfig.randomFn`, enabling deterministic testing with the conformance vectors.
...
This appendix provides a formal grammar for RDN notation per [RFC 5234](https://www.rfc-editor.org/rfc/rfc5234) (ABNF) with the case-sensitive string extension from [RFC 7405](https://www.rfc-editor.org/rfc/rfc7405). All alphabetic literals use `%i` (case-insensitive). This grammar assumes whitespace-stripped input (Section 8.2).
...
_End of RANDSUM Dice Notation Specification v0.9.0 rev.1_
