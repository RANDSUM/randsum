...
- `D{...}` after a core die expression is conditional Drop; `d{...}` at the start is Custom Faces
...
- **P6:** Implementations MUST strip all whitespace before grammatical parsing. Whitespace within annotation brackets is preserved.
...
;; Top-Level
rdn-input         = die-expression *( "," [WSP] die-expression )
...
mod-keep-middle   = %i"km" [positive-integer]
