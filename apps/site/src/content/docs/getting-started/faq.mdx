...
description: "Answers to common questions: why roll() never throws, seeded RNG for testing, browser support, reroll infinite loop prevention, and how drop vs. keep differ."
head:
  - tag: script
    attrs:
    ...
    content: |
      {
        ...
        "mainEntity": [
          {
            "@type": "Question",
            "name": "Why doesn't roll() throw errors?",
            "acceptedAnswer": {
              "@type": "Answer",
              "text": "RANDSUM's roll() is designed to never throw. Invalid input or runtime errors are returned in the result object as result.error, allowing you to handle failures gracefully without try/catch blocks."
            }
          },
          {
            "@type": "Question",
...
RANDSUM is not designed to throw — errors are returned as values rather than raised as exceptions. This makes error handling explicit and composable without try/catch blocks.
