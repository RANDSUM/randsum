...
description: Complete syntax reference for the Randsum Dice Notation standard. Covers NdS basics, drop/keep, cap, reroll, exploding, compound, penetrate, wild die, count successes/failures, integer division, modulo, geometric dice, draw dice, annotations, repeat operator, and modifier priority order.
head:
  - tag: script
    attrs:
...
Count dice at or below a threshold. Requires curly braces to avoid conflict with Fate dice.
...
<RollerPlayground defaultNotation="2d6+3[fire]+1d4[cold]" client:only="react" />
<RollerPlayground defaultNotation="4d6L[strength]" client:only="react" />
...
**Example:** `4d6Lx6` expands to six separate `4d6L` rolls -- perfect for D&D ability score generation.

## Special dice

### Geometric die

Roll dN until a 1 appears. Result is the attempt count.

| Notation | Description |
|---|---|
| `gN` | Geometric die (average: N rolls) |
...
Sample without replacement from a pool of faces (card-deck mechanic).

| Notation | Description |
|---|---|
| `DDN` | Draw one unique value from [1..N] |
| `3DDN` | Draw 3 unique values |
...
### D&D ability score generation (repeat)
<RollerPlayground defaultNotation="4d6Lx6" client:only="react" />
