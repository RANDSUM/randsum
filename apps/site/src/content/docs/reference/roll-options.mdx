...
The `roll()` function is the main entry point for all dice rolling in RANDSUM. It accepts several argument types and throws a `RandsumError` on invalid input.
...
`roll()` throws a `RandsumError` on invalid input. For user-provided notation strings, validate before rolling:

<CodeExample code={`import { validateNotation, isDiceNotation, roll, RandsumError } from '@randsum/roller'

// Validate before rolling (preferred for user input)
const validation = validateNotation(userInput)
if (validation.valid) {
  const result = roll(validation.argument) // use validation.argument (typed as DiceNotation)
}
...
:::tip
Hardcoded notation like `roll('4d6L')` will never throw in practice. Only validate when input comes from outside your control: form fields, API parameters, configuration files.
:::
