...
```typescript title="Character Stat Roll"
import { roll } from '@randsum/roller'
...
```typescript title="Advantage and Disadvantage"
import { roll } from '@randsum/roller'
...
```typescript title="Complex Notation"
import { roll } from '@randsum/roller'
...
result.rolls  // Array of RollRecord objects
```
...
if (isDiceNotation(userInput)) {
  roll(userInput) // userInput is typed as DiceNotation
}
