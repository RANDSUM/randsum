...
| `@randsum/roller/docs` | `NOTATION_DOCS`, `MODIFIER_DOCS`, `DICE_DOCS` and types `NotationDoc`, `ModifierDoc`, `ModifierCategory` | Build-time docs generation, introspection tooling |
...
import type { NotationDoc, ModifierDoc, ModifierCategory } from '@randsum/roller/docs'
```
...
`traceRoll(notation)` returns an array of `RollTraceStep` objects — a discriminated union with `kind: 'rolls' | 'divider' | 'arithmetic' | 'finalRolls'` — showing each stage of roll evaluation. `formatAsMath(notation)` returns a human-readable math string (e.g. `"2 + 4 = 6"`). Useful for verbose output displays and debugging.
