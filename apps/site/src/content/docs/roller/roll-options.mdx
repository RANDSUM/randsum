...
// Custom faces via notation
roll('d{fire,ice,lightning}')  // string faces
roll('d{2,3,5,7}')             // specific numeric faces

// Zero-bias dice via notation (faces 0 to N-1)
roll('z6')    // faces 0-5
roll('4z10')  // four dice with faces 0-9`} />
...
An object containing modifier options. See the [Modifiers](/roller/modifiers/) reference for all 15 available modifiers.
...
// Sort modifier (display-only, does not affect total)
roll({
  sides: 6,
  quantity: 6,
  modifiers: { sort: 'desc' }
