---
title: Modifiers
description: "Full TypeScript API for every RANDSUM modifier: cap, drop, keep, replace, reroll, explode, compound, penetrate, explode sequence, unique, count successes, and multipliers."
head:
  - tag: script
    attrs:
...
| 53 | Explode Sequence | `!s&#123;...&#125;` | `explodeSequence` |
| 60 | Unique | `U` | `unique` |
...
This means dice values are capped/constrained first, then pool size is adjusted (drop/keep), then values are replaced or rerolled, then explosive mechanics fire (including sequence explosions), and finally arithmetic is applied.
...
<CodeExample code={`interface ModifierOptions {
  ...
  explodeSequence?: ExplodeSequenceOptions
...
When a die shows its maximum value, a new die is rolled and added as a separate result. This is a single pass -- newly added dice are not checked for further explosions. Use compound (`!!`) or penetrate (`!p`) for recursive chaining.
...
roll('3d6!!0')   // Unlimited (capped at 1000)
...
roll('3d6!p0')   // Unlimited (capped at 1000)
...
Explode through a sequence of progressively different die sizes instead of re-rolling the same die.

**Options key:** `explodeSequence`

<CodeExample code={`import { roll } from '@randsum/roller'

// Explode through d4 -> d6 -> d8 (d8 repeats)
roll('1d4!s{4,6,8}')

// Inflation: explode UP through TTRPG standard set
roll('1d6!i')

// Reduction: explode DOWN through TTRPG standard set
roll('1d20!r')
...
roll({
  ...
  quantity: 1,
  modifiers: {
    explodeSequence: { sequence: [4, 6, 8] }
  }
})
...
When a die shows its maximum value, the next die in the sequence is rolled and added. The final die in the sequence repeats indefinitely (safety capped). The TTRPG standard die set is `[4, 6, 8, 10, 12, 20, 100]`.

All notation is **case-insensitive** (`!s` = `!S`, `!i` = `!I`, `!r` = `!R`).

**ExplodeSequenceOptions:**

<CodeExample code={`interface ExplodeSequenceOptions {
  sequence?: number[]
  inflation?: boolean
  reduction?: boolean
}`} />
...
7. Apply explode sequence (chain through die sizes)
8. Apply unique (ensure no duplicates)
9. Sum remaining dice
10. Apply pre-arithmetic multiply (`*`)
11. Apply plus/minus (`+`/`-`)
12. Count successes if applicable (S&#123;...&#125;)
13. Apply total multiply (`**`)
