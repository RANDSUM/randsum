---
title: Modifiers
description: "Full TypeScript API for every RANDSUM modifier: cap, drop, keep, replace, reroll, explode, compound, penetrate, wild die, unique, count successes, count failures, sort, integer divide, modulo, and multipliers."
head:
  - tag: script
    attrs:
...
**Example:** `5d10F{3}` rolls [8, 2, 10, 1, 9]. Failures at or below 3: [2, 1] = 2 failures.
...
7. Apply wild die behavior (compound on max, remove on 1)
8. Apply unique (ensure no duplicates)
9. Sum remaining dice
10. Apply pre-arithmetic multiply (`*`)
11. Apply plus/minus (`+`/`-`)
12. Sort results if applicable (`sa`/`sd`)
13. Apply integer divide (`//N`)
14. Apply modulo (`%N`)
15. Count successes/failures if applicable (S&#123;...&#125;/F&#123;...&#125;)
