---
title: Modifiers
description: "Full TypeScript API for every RANDSUM modifier: cap, drop, keep, replace, reroll, explode, compound, penetrate, unique, count successes, and multipliers."
head:
  - tag: script
    attrs:
...
Modifiers transform dice results after rolling. They can drop dice, reroll values, cap ranges, make dice explode, sort results, and more. There are 15 modifiers in total, applied in a fixed priority order to ensure consistent, predictable results.
...
| 10 | Cap | C&#123;...&#125; | `cap` |
| 20 | Drop | `H`, `L`, D&#123;...&#125; | `drop` |
| 21 | Keep | `K`, `kl`, `KM` | `keep` |
| 30 | Replace | V&#123;...&#125; | `replace` |
| 40 | Reroll | R&#123;...&#125;, `ro`&#123;...&#125; | `reroll` |
| 50 | Explode | `!` | `explode` |
...
| 90 | Plus | `+N` | `plus` |
| 91 | Minus | `-N`, `ms`&#123;N&#125; | `minus` |
| 92 | Sort | `s`, `sa`, `sd` | `sort` |
...
// Keep middle
roll('6d6KM')  // Drop 1 from each end, keep middle 4
roll('6d6KM2') // Drop 2 from each end, keep middle 2
...
:::note
`KM` (keep middle) is sugar for dropping N from each end. `6d6KM` drops 1 lowest and 1 highest, keeping the middle 4 dice.
:::
...
`ro{...}` is sugar for `R{...}1` -- reroll matching dice at most once.

<CodeExample code={`import { roll } from '@randsum/roller'

roll('4d20ro{<5}')  // Reroll under 5, once only (same as R{<5}1)`} />
...
Sort the displayed dice results. Sorting is display-only -- it does not affect the total or any other modifier.

**Options key:** `sort`

<CodeExample code={`import { roll } from '@randsum/roller'

roll('6d6s')   // Sort ascending (default)
roll('6d6sa')  // Sort ascending (explicit)
...
:::note
Sort (`s`/`sa`/`sd`) does not conflict with count successes (`S{7}`). The lowercase sort tokens are distinct from the uppercase `S{...}` syntax.
:::
...
10. Apply plus/minus (`+`/`-`)
11. Apply sort (display-only ordering)
