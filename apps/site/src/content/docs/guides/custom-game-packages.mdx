---
title: Build a Custom RPG Dice System with createGameRoll
description: Step-by-step guide to building your own typed dice system using createGameRoll. Covers validate, toRollOptions, interpretResult, and multi-roll game mechanics.
---
...
const myRoll = createGameRoll({
  ...
  interpretResult: (input, fullResult) => { /* return result */ }
})`} />
...
const rollPbta = createGameRoll({
  ...
  interpretResult: (_input, fullResult) => {
    if (fullResult.total >= 10) return 'strong_hit'
    if (fullResult.total >= 7) return 'weak_hit'
    return 'miss'
  }
})
...
Receives the input and the full `RollerRollResult`. Return your game-specific `TResult` here.
...
const rollPbta = createGameRoll({
  ...
  computeDetails: (input, fullResult) => ({
    diceValues: fullResult.rolls.flatMap(r => r.rolls),
    statBonus: input
  })
})
...
Keep your custom package under 8 KB (gzipped) — the same limit enforced for all official `@randsum` game packages.
