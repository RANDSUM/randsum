...
Game packages (`@randsum/games/*`) are code-generated from `.randsum.json` specs and call `roll()` from `@randsum/roller` internally. They do not expose a `randomFn` parameter.
