...
5. **Write tests**
    ...
    Create two test files in `packages/games/__tests__/`:
...
describe('roll property-based tests', () => {
  test('result is always a valid outcome', () => {
    ...
    Adapt these templates to your game's inputs and outcomes. See existing test files (e.g., `blades.test.ts`, `fifth.test.ts`) for patterns handling game-specific inputs like `rating` or `modifier`.
...
- **Check existing specs.** The six built-in game specs in `packages/games/` cover a range of mechanics — pool-based (blades), advantage/disadvantage (fifth), dual-pool (daggerheart), 2d6+stat (pbta), and large outcome tables (salvageunion). Find the one closest to your game and use it as a starting point.
