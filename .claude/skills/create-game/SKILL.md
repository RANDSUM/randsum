...
This creates the package structure at `packages/<name>/`.

## Post-scaffold Guidance

After the script runs:

1. **Verify structure**: confirm `packages/<name>/` has `src/`, `__tests__/`, `package.json`, `CLAUDE.md`
...
4. **Implement roll function**: Edit `src/roll<GameName>/index.ts` using the `createGameRoll` factory:
   ...
   ```
5. **Write tests**: Add tests in `__tests__/` following existing game package patterns (see `packages/blades/__tests__/` or `packages/fifth/__tests__/` for examples)
