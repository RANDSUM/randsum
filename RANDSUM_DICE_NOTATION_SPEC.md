...
| **Alias**                  | A notation convenience that desugars to one or more primitives with identical observable behavior.                                                                           |
...
| **Alias**     | Desugars to one or more primitives. Observable behavior is identical to the desugared form. | Closed |
...
| ----- | --------------------- | -------------- | ----------------------------------------------------------------------------------- | ------------------------ |
| 1     | Deterministic Shaping | 10--39         | No RNG. Pure functions over the dice array. `requiresRollFn = false`.               | `requiresRollFn = false` |
| 2     | Stochastic Dynamics   | 40--69         | Requires a roll function. May introduce new random values. `requiresRollFn = true`. | `requiresRollFn = true`  |
| 3     | Total Derivation      | 80--127        | Pool is frozen. Operates on the scalar total only. `mutatesRolls = false`.          | `mutatesRolls = false`   |
...
| **Filter**      | 1     | Remove dice from the pool. Pool shrinks.                            |
...
3. **Aliases inherit facets from their parent primitive.** An alias's Stage, Verb, and Channel are identical to the primitive it desugars to. Keep inherits Stage 1 / Filter / Pool from Drop. Minus inherits Stage 3 / Scale / Total from Plus.
...
| ----- | ----------- | ------- | -------------------------------------------------------------------------------------------------------- |
| 1     | Clamp       | Pool    | Cap                                                                                                      |
| 1     | Filter      | Pool    | Drop, Keep (alias), Keep Middle (alias)                                                                  |
| 1     | Map         | Pool    | Replace                                                                                                  |
...
Sort (Stage 3, priority 92) is a **Presentation Directive**, not an operational modifier. It reorders the pool array for display but does not alter the total. It is classified under the Pool channel as a special case: it mutates the array ordering but not the array contents or the total.
...
**Desugars to:** Standard die `1d100`.
...
**Fate Core (dF):** Desugars to `d3` with Replace modifier `V{1=-1,2=0,3=1}`. Produces values in {-1, 0, +1}.

**Extended Fudge (dF.2):** Desugars to `d5` with Replace modifier `V{1=-2,2=-1,3=0,4=1,5=2}`. Produces values in {-2, -1, 0, +1, +2}.
...
**Desugars to:** Custom Faces die `d{0,1,2,...,N-1}`. Produces values in [0..N-1] instead of [1..N].
...
1. **Stage 1 -- Deterministic Pool Shaping** (priority 10--39): Pure transformations of the pool array. No random number generation.
2. **Stage 2 -- Stochastic Pool Dynamics** (priority 40--69): Transformations that introduce randomness. Requires a roll function.
3. **Stage 3 -- Total Derivation** (priority 80--127): Pool is frozen. Operates on the scalar total.
...
`Primitive | Stage 1 | Filter | Pool channel | Priority 20`
...
- MUST have access to a roll function (`requiresRollFn = true`).
...
Each alias desugars to a normative primitive form. An alias MUST produce identical observable behavior to its desugared form.

#### 6.7.1 Keep

**Notation:** `K`, `Kn`, `kl`, `klN`

**Desugars to:** Drop (inverse).

Keeping the highest N dice from a pool of Q dice is equivalent to dropping the lowest (Q - N) dice. `KN` on a pool of Q dice desugars to `L(Q-N)`. `klN` (keep lowest) desugars to `H(Q-N)`.

When N is omitted, it defaults to 1.

**Inherits:** Stage 1 | Filter | Pool channel | Priority 21.

#### 6.7.2 Keep Middle

**Notation:** `KM`

**Desugars to:** Drop lowest 1 + Drop highest 1.
...
**Desugars to:** Plus with negated operand. `-N` is equivalent to `+(-N)`.
...
**Desugars to:** Minus N, which itself desugars to Plus (-N). This is a transitive desugaring chain: `ms{N}` -> `-N` -> `+(-N)`.
...
**Desugars to:** Multiply at priority 100 (post-arithmetic phase).
...
**Desugars to:** Count with `greaterThanOrEqual: N`. Optionally `S{N,B}` desugars to Count with `greaterThanOrEqual: N, lessThanOrEqual: B, deduct: true`.
...
**Desugars to:** Count with `lessThanOrEqual: N`.
...
**Desugars to:** Explode Sequence with the ascending TTRPG standard die set: `!s{4,6,8,10,12,20,100}`.
...
**Desugars to:** Explode Sequence with the descending TTRPG standard die set: `!s{100,20,12,10,8,6,4}`.
...
**Explosion + Filter:** Explosions (Stage 2, priority 50--55) run after filters (Stage 1, priority 20--21). Dropped dice do not trigger explosions because they have already been removed from the pool. Example: `4d6L!` drops the lowest die first, then checks the remaining three dice for explosions.

**Cap + Drop:** Cap (priority 10) runs before Drop (priority 20). A value that would have been dropped may first be capped, potentially changing which dice are "lowest" or "highest". Example: `4d6C{>5}L` caps all values at 5, then drops the lowest of the capped results.
...
4d6L!+2    -- Drop lowest, explode, add 2
            -- Execution: Drop(20) -> Explode(50) -> Plus(90)
```
...
| 20       | Drop             | 1     | Filter      | Pool    | Primitive |
| 21       | Keep             | 1     | Filter      | Pool    | Alias     |
...
| Alias             | Notation  | Primitive        | Desugaring Rule                            |
| ----------------- | --------- | ---------------- | ------------------------------------------ |
...
_End of Randsum Dice Notation Specification v1.0_
